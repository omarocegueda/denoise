...
cpdef ornlmpy(double[:, :, :] image, int volumeLen, int patchLen, double h):
...
cpdef Average_block_cpp(double[:, :, :] image, int x, int y, int z, double[:, :, :] av, double weight):
    cdef double[:,:,:] I=image.copy_fortran()
    cdef double[:,:,:] average=av.copy_fortran()
    cdef int[:] dims=cvarray((3,), itemsize=sizeof(int), format="i")
    dims[0]=I.shape[0]
    dims[1]=I.shape[1]
    dims[2]=I.shape[2]
    neighSize=average.shape[0]//2
    Average_block(&I[0,0,0], x, y, z,neighSize, &average[0,0,0], weight, &dims[0])
    av[:,:,:]=average[:,:,:]

cpdef Value_block_cpp(double[:, :, :] Estimate, double[:, :, :] Label, x, y, z, double[:, :, :] av, double global_sum, double hh):
    cdef double[:,:,:] E=Estimate.copy_fortran()
    cdef double[:,:,:] L=Label.copy_fortran()
    cdef double[:,:,:] average=av.copy_fortran()
    cdef int[:] dims=cvarray((3,), itemsize=sizeof(int), format="i")
    dims[0]=E.shape[0]
    dims[1]=E.shape[1]
    dims[2]=E.shape[2]
    neighSize=average.shape[0]//2
    Value_block(&E[0,0,0], &L[0,0,0], x, y, z, neighSize, &average[0,0,0], global_sum, &dims[0], hh)
    Estimate[:,:,:]=E[:,:,:];
    Label[:,:,:]=L[:,:,:];

cpdef distance_cpp(double[:, :, :] image, int x,int y,int z, int nx, int ny, int nz, int f):
    cdef double[:,:,:] I=image.copy_fortran()
    cdef int[:] dims=cvarray((3,), itemsize=sizeof(int), format="i")
    dims[0]=I.shape[0]
    dims[1]=I.shape[1]
    dims[2]=I.shape[2]
    retVal=distance(&I[0,0,0], x, y, z, nx, ny, nz, f, dims[0], dims[1], dims[2])
    return retVal
